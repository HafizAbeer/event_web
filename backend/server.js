...
const mongoUrl =
  "mongodb+srv://hafizabeer997outlook:IE89xO8NySSALkMs@cluster0.ugqgo.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";

const JWT_SECRET =
  "hvdvay6ert72839289()aiyg8t87qt72393293883uhefiuh78ttq3ifi78272jdsds047[]]pou89ywe";
...
app.post("/api/create", async (req, res) => {
  const { title, description, location, date, category, visibility, token } =
    req.body;
...
app.put("/api/edit-event/:id", async (req, res) => {
  const eventId = req.params.id; // Event ID from the URL
  const { token, title, description, location, date, category, visibility } =
    req.body; // Extract event data and token
